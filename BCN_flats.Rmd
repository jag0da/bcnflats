...
```
###Map
```{r map1, warning=F, message=F, echo=F}
bcn_shp <- "bcn_district/bcn_districte_ed50_shp.shp"
bcn_geo <- st_read(bcn_shp)

#fixing codes in data table to factor in same level as 
...
ggplot(dist_data) + 
  ...
  scale_fill_distiller(palette="Reds", direction=1, name="Nr of tourist flats") +
  labs(title="How many tourist flats exist for every 100 local flats?", subtitle="in each district of Barcelona", caption="Source: Barcelona's City Hall Open Data Service")+
  theme_void()+
